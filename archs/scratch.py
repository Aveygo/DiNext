...
class DiT(nn.Module):
    ...
    def forward(self, x, t=None, y=None):
        ...
        """
        if t is None:
            t = torch.zeros((x.shape[0],)).to(x.device).long()

        if y is None:
            y = torch.zeros((x.shape[0],)).to(x.device).long()
...
def DiT_XL_2(**kwargs):
    return DiT(depth=28, hidden_size=1152, num_heads=16, patch_size=2)

def DiT_B_2(**kwargs):
    return DiT(depth=12, hidden_size=768, num_heads=12, patch_size=2)

def DiT_S_2(**kwargs):
    return DiT(depth=12, hidden_size=384, num_heads=6, patch_size=2)
